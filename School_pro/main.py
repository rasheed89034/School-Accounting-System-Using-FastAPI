...
@app.get("/classes-page")
def classes_page(request: Request, db: Session = Depends(get_db)):
    return templates.TemplateResponse(request=request, name="classes.html", context={"request": request, "classes": db.query(model.ClassGrade).all()})

@app.get("/students-page")
def students_page(request: Request, db: Session = Depends(get_db)):
    return templates.TemplateResponse(request=request, name="students.html", context={"request": request, "students": db.query(model.Student).all(), "classes": db.query(model.ClassGrade).all()})

@app.get("/receive-fee-page")
def receive_fee_page(request: Request, db: Session = Depends(get_db)):
    return templates.TemplateResponse(request=request, name="receive_fee.html", context={"request": request, "invoices": db.query(model.FeeInvoice).all(), "students": db.query(model.Student).all()})

@app.get("/employees-page")
def employees_page(request: Request, db: Session = Depends(get_db)):
    return templates.TemplateResponse(request=request, name="employees.html", context={"request": request, "employees": db.query(model.Employee).all()})

@app.get("/expenses-page")
def expenses_page(request: Request, db: Session = Depends(get_db)):
    return templates.TemplateResponse(request=request, name="expenses.html", context={"request": request, "expenses": db.query(model.Expense).all()})

# --- Search Portal Routes ---
@app.get("/search-page")
def search_page(request: Request):
    return templates.TemplateResponse(request=request, name="search.html", context={"request": request, "student_result": None, "employee_result": None, "error": None})

@app.post("/search-student")
def search_student(request: Request, roll_number: str = Form(...), db: Session = Depends(get_db)):
    student = db.query(model.Student).filter(model.Student.roll_number == roll_number).first()
    if not student:
        return templates.TemplateResponse(request=request, name="search.html", context={"request": request, "student_result": None, "employee_result": None, "error": "Student not found!"})
    invoices = db.query(model.FeeInvoice).filter(model.FeeInvoice.student_id == student.id).all()
    student_data = {"info": student, "invoices": invoices}
    return templates.TemplateResponse(request=request, name="search.html", context={"request": request, "student_result": student_data, "employee_result": None, "error": None})

@app.post("/search-employee")
def search_employee(request: Request, emp_id: int = Form(...), db: Session = Depends(get_db)):
    employee = db.query(model.Employee).filter(model.Employee.id == emp_id).first()
    if not employee:
        return templates.TemplateResponse(request=request, name="search.html", context={"request": request, "student_result": None, "employee_result": None, "error": "Teacher/Staff not found!"})
    return templates.TemplateResponse(request=request, name="search.html", context={"request": request, "student_result": None, "employee_result": employee, "error": None})


# --- Form Handlers (Data Save Karne Wale Routes) ---
@app.post("/submit-class")
def submit_class(class_name: str = Form(...), monthly_tuition_fee: float = Form(...), db: Session = Depends(get_db)):
    db.add(model.ClassGrade(class_name=class_name, monthly_tuition_fee=monthly_tuition_fee))
    db.commit()
